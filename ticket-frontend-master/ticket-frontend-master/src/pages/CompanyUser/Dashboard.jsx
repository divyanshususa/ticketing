...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await Axios.get("/api/ticket/getCompanyTicket", {
        headers: {
  ...
  const fetchDashboardData = async () => {
    try {
      // Set up Axios instance with credentials option to include cookies
      const res = await Axios.get("/api/dashboard/companyDashboardData", {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
      ...
      {data2.length >= 0 ? (
        <>
          <div className="flex justify-between">
            ...
            <button
              onClick={showModal}
              className="bg-blue-600 p-2 rounded-md uppercase text-white "
            >
              create tickets
            </button>
          </div>
          <div className="flex justify-between">
            ...
            <Widgets
              title="open"
              number={
                dashboardData.open_tickets === null
                  ? 0
